...
system(paste0("module load bioinfo/samtools/1.19; samtools view -F 0x104 ", sam , " | awk '{print $1, $3, $4, length($10)}' > fq_fa_fqstart.txt" ), intern = TRUE)
...
colnames(main) <- c("contigID", "readID", "fa_wst", "fa_wen", "fq_wst", "fq_wen", "fa_length", "fq_length", "read_length", "contig_start")
# fq region = fa region:
...
for (n in c(5000, 100, 50, 10)) {
	...
	pdf(paste0(spec,"_facet_bridging_", n , "bp.pdf"), width=13.8, height=7.31, onefile=TRUE)
	...
	system(paste0("convert ",spec,"_facet_bridging_", n , "bp.pdf ",spec,"_facet_bridging_", n , "bp.png"), intern = TRUE)
}
