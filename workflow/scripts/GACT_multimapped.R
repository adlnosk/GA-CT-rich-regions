...
# reads multimapped (primary and secondary => without unmapped and supplementary = 800 + 4) with soft/hard clip
...
sam = paste0("/work/project/briefwp3/Adela/", spec, "/assembly/map_raw/", spec, "_mapped_sorted.bam")
print(sam)

system(paste0("module load bioinfo/samtools/1.19; samtools view -F 0x104 ", sam , " | awk '$6 ~ /H|S/{print $0}' | awk '{print $1, $2, $3, $4, $6, length ($10)-1 }'  > multimapped_fq_fa_pos.txt"), intern = TRUE)
...
write.table(targets[,c("readID", "contigID", "start", "end", "contig_start", "leftclip", "rightclip")], paste0("targets_", spec, "_multimapped_withCIGAR.txt"), col.names=T, row.names=F, quote=F)  
...
write.table(locs, "locs", col.names=F, row.names=F, quote=F)
